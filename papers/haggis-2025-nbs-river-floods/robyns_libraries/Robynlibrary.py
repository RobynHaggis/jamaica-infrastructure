...
def rasterize_condition(geodf, out_file, resolution=10):
    ...
    shapes = ((geom, value) for geom, value in zip(geodf.geometry, geodf['condition_factor_p4'])) #condition_factor_p4 used to be just Condition without the p4
...
def calc_connectivity_percentage(baseline, max_val, min_val):
    """
    Calculate the connectivity percentage based on a baseline value, 
    where max_val is 100% connectivity and min_val is 0% connectivity.
    ...
    Returns:
        float: The connectivity percentage of the baseline value.
    """
    percentage = ((baseline - min_val) / (max_val - min_val)) * 100
    return percentage
